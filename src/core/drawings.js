...
import { SHEET_STYLE, HATCH_DEF, esc } from '../draw2d/svg.js';
...
export function buildSheetSvg({ renderer, scene, camera, controls, projection, sceneState }) {
  ...
  const height = TITLE_H + cell * 3 + PAD * 4;
  ...
  layout.forEach((row, r) =>
    row.forEach((view, c) => {
      ...
      let body;
      if (isWall) body = elevationGroup(sceneState, view, cw, ch);
      else if (view === 'plan') body = planGroup(sceneState, cw, ch);
